...
function doRectanglesOverlap(rect1, rect2) {
  const bottomBorder = rect1.top + rect1.height;
  const rightBorder = rect1.left + rect1.width;
  if (bottomBorder > rect2.top && rightBorder > rect2.left) {
    return true;
  }
  return false;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; !done && i < pathes[0].length; i += 1) {
    const char = pathes[0][i];
    for (let j = 1; j < pathes.length; j += 1) {
      if (i >= pathes[j].length || pathes[j][i] !== char) {
        done = true;
        break;
      }
    }
