...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = difference.getUTCHours().toString().padStart(2, '0');
  const minutes = difference.getUTCMinutes().toString().padStart(2, '0');
  const seconds = difference.getUTCSeconds().toString().padStart(2, '0');
  const milliseconds = difference.getUTCMilliseconds().toString().padStart(3, '0');
...
function angleBetweenClockHands(date) {
  ...
  const hourPosition = minutes * 0.5 + hours * 30;
  let diff = Math.abs(minutePosition - hourPosition);
  if (diff > 180) {
    diff = 360 - diff;
  }
